...
router.route("/").post((req, res) => {
  ...
  if (username.length < 5) {
    ...
    res.json({
      success: false,
      message: `Username length can't be lower than five!`
    });
    return;
  }
              ...
              Session.save((err4, session_result) => {
                ...
                request("http://localhost:5000/update/getdate", function(
                  ...
                  if (err5_5) {
                    ...
                    res.json({
                      success: false,
                      message: `Server Error! 'v005_5'`
                    });
                    return;
                  }
                  let lastLogin = JSON.parse(body).result;
                  findOne_result.updateOne(
                    { lastlogin: lastLogin },
                    (err5, update_result) => {
                      if (err5) {
                        console.log(
                          `findOne_result.updateOne() Error: '${err5}'!`
                        );
                        res.json({
                          success: false,
                          message: `Server Error! '005'`
                        });
                        return;
                      }
                      res.json({
    ...
    UserSession.findOne(
      ...
      (err10, usessionfindOne_result) => {
        ...
        if (usessionfindOne_result === null) {
          console.log(
            `UserSession.findOne() returned null! userId: '${userId}'!`
          );
        ...
        if (tokenFromDb != token) {
          console.log(
            `Client and Server session ID mismatch: ${tokenFromDb} <> ${token}!`
          );
