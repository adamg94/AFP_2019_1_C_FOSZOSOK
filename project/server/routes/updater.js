...
router.route('/villageupdate').post((req, res) => {
	Village.findOne({ userId: req.body.userId }, (err2, village_findOne_result) => {
		if (err2) {
		...
		if (village_findOne_result) {
           ...
           */
				...
				const timeSinceLastUpdate = parseFloat(
					(Date.parse(newdate) - Date.parse(village_findOne_result.lastupdate)) / 1000
				);

				//fa
				village_findOne_result.buildings.warehouse.wood +=
					GAMESETTINGS.BASE_MULTIPLIER *
					(timeSinceLastUpdate * (village_findOne_result.buildings.lumberyard.level * 80 / 3600));

				//agyag
				village_findOne_result.buildings.warehouse.brick +=
					GAMESETTINGS.BASE_MULTIPLIER *
					(timeSinceLastUpdate * (village_findOne_result.buildings.brickyard.level * 80 / 3600));
				...
				village_findOne_result.buildings.warehouse.iron +=
					GAMESETTINGS.BASE_MULTIPLIER *
					(timeSinceLastUpdate * (village_findOne_result.buildings.ironmine.level * 80 / 3600));
