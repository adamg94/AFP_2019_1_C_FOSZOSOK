              ...
              Village.findOne(
                ...
                async (err3, foundVillage) => {
                  ...
                  if (
                    ...
                    ezkell +
                      (7800000 -
                        600000 * foundVillage.buildings.temple.level) <=
      ...
      if (req_res.body.success) {
        ...
        request.post(
          ...
          (err8, req_update_res) => {
            ...
            if (req_update_res.body.success) {
              Construct.find(
                { villageId: req_update_res.body.village._id },
                (err14, constructfind_result) => {
                  if (err14) {
                    console.log(`Construct.find Error: '${err14}'!`);
                    res.json({
                      success: false,
                      message: `Server Error! '014'`
                    });
                    return;
                  }
                  
                  let canIBuild = true;
                  for (let i = 0; i < constructfind_result.length; i++) {
                    if (constructfind_result[i].buildingId == id) {
                      canIBuild = false;
                    }
                  }
                  ...
                  if (!canIBuild) {
                    res.json({
                      success: false,
                      message: `You can't build the same building until the current one finishes.`,
                      village: req_update_res.body.village
                    });
                    return;
                  }
                 
                  if (constructfind_result.length < 3 && canIBuild) {
                      ...
                      async (err9, findByResult) => {
                        ...
                        if (id == 1) {
                          if (building.level > 0 && building.level <= 20) {
                            constructionTime = building.level * 5;
                            constructionPrice = building.level * 50;
                          } else if (
                            building.level > 20 &&
                            building.level <= 40
                          ) {
                            constructionTime = building.level * 10;
                            constructionPrice = building.level * 500;
                          } else {
                            constructionTime = building.level * 20;
                        ...
                        if (
                          ...
                          findByResult.buildings.warehouse.wheat -=
                            constructionPrice / 10;
                          ...
                          let save_result = await findByResult
                            ...
                            .then(_ => {
                              ...
                              request(
                                "http://localhost:5000/update/getdate",
                                function(err13, response, body) {
                                  if (err13) {
                                    console.log(
                                      `NTP Server call Error: '${err13}'!`
                                    );
                                    res.json({
                                      success: false,
                                      message: `Server Error! 'v013'`
                                    });
                                    return;
                                  }
                                  newdate = JSON.parse(body).result;
                                  
                                  endtime = constructionTime * 60000;


                                  if(constructfind_result.length > 0)
                                    for (let i = 0; i < constructfind_result.length; i++) 
                                      newdate = constructfind_result[i].jobTime
                                  
                                  newdate = Date.parse(newdate);
                                  const construct = new Construct({
                                    villageId: findByResult._id,
                                    buildingId: id,
                                    action: 1,
                                    startTime: new Date(newdate),
                                    jobTime: new Date(newdate + endtime)
                                  });

                                  construct.save((err9, save_result) => {
                                    if (err8) {
                                      console.log(
                                        `newConstruct.save() Error: '${err8}'!`
                                      );
                                      res.json({
                                        success: false,
                                        message: `Server Error! '009'`
                                      });
                                      return;
                                    }
                    ...
                    res.json({
                      success: false,
                      message: `Village has a three building queue`,
                      village: req_update_res.body.village
                    });
                    return;
                  }
                  ...
                  if (constructfind_result.length < 3 && canIBuild) {
                    ...
                    Village.findById(
                      ...
                      async (err9, findByResult) => {
                        ...
                        let stone = findByResult.buildings.warehouse.stone;
                        let metal = findByResult.buildings.warehouse.metal;
                        let wheat = findByResult.buildings.warehouse.wheat;


                        if (
                          stone - ((constructionPrice * 3) * 0.05) >= 0 &&
                          metal - ((constructionPrice * 3) * 0.05) >= 0 &&
                          wheat - constructionPrice / 10 >= 0
                        ) {
                          findByResult.buildings.warehouse.stone -= ((constructionPrice * 3) * 0.05);
                          findByResult.buildings.warehouse.metal -= ((constructionPrice * 3) * 0.05);
                          findByResult.buildings.warehouse.wheat -=
                            constructionPrice / 10;
